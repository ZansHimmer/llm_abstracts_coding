import os
import requests
import json

# version 04/03/2026

from master_config import SETTINGS, PROMPT_FILE, PAPERS_FILE


# CONFIG
START = 0
END = 1000
VERSION = SETTINGS.version
BATCH_SIZE = SETTINGS.batch_size
MODEL = SETTINGS.model
SAFE_MODEL = SETTINGS.safe_model

OUTPUTS_DIR = SETTINGS.outputs_dir()
RESPONSES_DIR = SETTINGS.responses_dir()

# Load prompt
with open(PROMPT_FILE, "r", encoding="utf-8") as f:
    prompt_text = f.read().strip()

# Load papers
with open(PAPERS_FILE, "r", encoding="utf-8") as f:
    papers_text = f.read().strip()


def build_prompt_with_papers(prompt, papers_text, start_idx=0, end_idx=5):
    paper_blocks = papers_text.strip().split("\n\n")
    selected_papers = paper_blocks[start_idx:end_idx]
    return prompt + "\n\n" + "\n\n".join(selected_papers)


def call_ollama(prompt):
    response = requests.post(
        "http://localhost:11434/api/generate",
        json={
            "model": MODEL,
            "prompt": prompt,
            "stream": False,
            "options": {
            "temperature": (SETTINGS.temperature if SETTINGS.temperature is not None else 1.0)
            }
        },
        timeout=600,
    )
    response.raise_for_status()
    return response.json()


def main():
    OUTPUTS_DIR.mkdir(parents=True, exist_ok=True)
    RESPONSES_DIR.mkdir(parents=True, exist_ok=True)

    for i in range(START, END, BATCH_SIZE):
        batch_start = i
        batch_end = min(i + BATCH_SIZE, END)

        full_prompt = build_prompt_with_papers(
            prompt_text, papers_text,
            start_idx=batch_start,
            end_idx=batch_end
        )

        print(f"Processing papers {batch_start + 1}–{batch_end}")

        try:
            ollama_json = call_ollama(full_prompt)
            output_text = ollama_json["response"]
        except Exception as e:
            print(f"❌ failed for batch {batch_start + 1}-{batch_end}: {e}")
            continue

        with open(
            str(OUTPUTS_DIR / f"{batch_start + 1}_{batch_end}_coding_output_{VERSION}.txt"),
            "w",
            encoding="utf-8",
        ) as f:
            f.write(output_text)

        with open(
            str(RESPONSES_DIR / f"{batch_start + 1}_{batch_end}_coding_response_{VERSION}.txt"),
            "w",
            encoding="utf-8",
        ) as f:
            json.dump(ollama_json, f, indent=2)

    print("✅ Done")


if __name__ == "__main__":
    main()